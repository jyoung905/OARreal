      ...
      <header style={{position:'sticky',top:0,width:'100%',zIndex:50,background:'rgba(6,11,22,0.92)',backdropFilter:'blur(14px)',borderBottom:'1px solid rgba(255,255,255,0.05)'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'1rem 1.5rem',maxWidth:'80rem',margin:'0 auto'}}>
          ...
          <nav className="rh-desktop-nav" style={{display:'flex',alignItems:'center',gap:'2rem'}}>
            {navLinks.map(l => (
              <Link key={l.t} href={l.h} style={{color: l.h === '/resources' ? GOLD : 'rgba(255,255,255,0.7)',fontWeight: l.h === '/resources' ? 700 : 500,fontSize:'0.875rem',textDecoration:'none',fontFamily:'Inter,sans-serif'}}>{l.t}</Link>
            ))}
          </nav>
          <div style={{display:'flex',alignItems:'center',gap:'0.75rem'}}>
            <a href="/#intake" className="rh-header-cta" style={{background:GOLD,color:'#1a0f00',fontWeight:700,padding:'0.625rem 1.5rem',borderRadius:'0.375rem',fontSize:'0.875rem',textDecoration:'none',fontFamily:'Inter,sans-serif'}}>Get My Free Review</a>
        ...
        {open && (
          <nav className="rh-mobile-menu" style={{display:'flex',flexDirection:'column',gap:'0.875rem',padding:'1.25rem 1.5rem 1.5rem',background:'#060b16',borderTop:'1px solid rgba(203,167,47,0.2)'}}>
            {navLinks.map(l => (
              <Link key={l.t} href={l.h} onClick={() => setOpen(false)} style={{color:'rgba(255,255,255,0.85)',fontWeight:500,fontSize:'1rem',textDecoration:'none',padding:'0.25rem 0',fontFamily:'Inter,sans-serif'}}>{l.t}</Link>
            ))}
            <a href="/#intake" onClick={() => setOpen(false)} style={{display:'block',textAlign:'center',marginTop:'0.5rem',padding:'0.875rem 1.5rem',background:GOLD,color:'#1a0f00',fontWeight:700,borderRadius:'0.375rem',fontSize:'0.95rem',textDecoration:'none',fontFamily:'Inter,sans-serif'}}>Get My Free Claim Review</a>
          </nav>
        )}
      </header>
    </>
