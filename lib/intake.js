...
export function sanitizeLead(payload = {}) {
  const data = {
    ...
    consentReferralShare: Boolean(payload.consentReferralShare),
  ...
  data.consentReferralShare = payload.consentReferralShare === undefined ? false : data.consentReferralShare;

  if (data.consentToContact !== true) {
    throw new Error('Missing required field: contact consent');
  }
