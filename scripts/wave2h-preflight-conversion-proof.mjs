...
const DEBUG_PORT = Number(process.env.WAVE2H_DEBUG_PORT || 9242);
...
async function run() {
  ...
  try {
    ...
    let p = await page(cdp, `${BASE}/thank-you?wave2h_direct=${Date.now()}`, 1440, 900);
    ...
    p = await page(cdp, `${BASE}/?wave2h_seed=${Date.now()}`, 1440, 900);
    const marker = `wave2h-preflight-${Date.now()}`;
    await evalOn(cdp, p, `sessionStorage.setItem('oar_lead_conversion_pending', ${JSON.stringify(marker)}); location.href='/thank-you?wave2h_marker=1'; true`);
    await delay(4500);
