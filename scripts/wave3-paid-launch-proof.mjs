...
const DEBUG_PORT = Number(process.env.WAVE3_DEBUG_PORT || 9260);
const CAMPAIGN = 'wave3_test';
const UTM = `utm_source=google&utm_medium=cpc&utm_campaign=${CAMPAIGN}&utm_term=ontario_accident_benefits&utm_content=control`;
const MARKER_PREFIX = 'OAR Wave 3 QA DELETE';
const UNIQUE = `${MARKER_PREFIX} ${Date.now()}`;
const FAKE_EMAIL = 'oar-wave-3-delete@ontarioaccidentreview.ca';
...
async function analyticsPreflight() {
  ...
  const marker = `wave3-preflight-track-${Date.now()}`;
  ...
  const payload = { event: 'wave3_preflight_track', device: 'server', traffic_source: 'wave2g', utm_medium: 'qa', utm_campaign: CAMPAIGN, landing_page: '/wave2g-preflight', trigger: marker, props: { marker } };
  ...
  await delay(2500);
  const stored = await jfetch(`${SUPA}/rest/v1/${ANALYTICS_TABLE}?event=eq.wave3_preflight_track&trigger=eq.${encodeURIComponent(marker)}&select=id,event,trigger,utm_campaign`, { headers });
  ...
  const verify = await jfetch(`${SUPA}/rest/v1/${ANALYTICS_TABLE}?event=eq.wave3_preflight_track&trigger=eq.${encodeURIComponent(marker)}&select=id`, { headers });
...
async function harnessPreflight(cdp) {
  const p = await page(cdp, `${BASE}/thank-you?preflight=wave3`, 1280, 820);
...
async function finalProof(cdp) {
  ...
  try {
    let p = await page(cdp, `${BASE}/?${UTM}`, 1440, 1000);
    await shot(cdp, p, 'homepage-top-fold-desktop');
    await close(cdp, p);

    p = await page(cdp, `${BASE}/?${UTM}`, 390, 900);
    await shot(cdp, p, 'homepage-top-fold-mobile');
    await close(cdp, p);

    p = await page(cdp, `${BASE}/?${UTM}#intake`, 1440, 1000);
    await shot(cdp, p, 'intake-first-step-desktop');
    ...
    await evalOn(cdp, p, fill('#im-msg', `${UNIQUE}. QA verification only. No real accident. Delete after Wave 3 verification.`));
    await evalOn(cdp, p, clickAction('Continue'));
    await delay(600);
    await shot(cdp, p, 'intake-final-step-desktop');
    await close(cdp, p);

    p = await page(cdp, `${BASE}/?${UTM}#intake`, 390, 900);
    await shot(cdp, p, 'intake-first-step-mobile');
    await close(cdp, p);

    p = await page(cdp, `${BASE}/thank-you?${UTM}`, 1440, 900);
    ...
    await evalOn(cdp, p, clickText('Yes'));
    await evalOn(cdp, p, clickText('Insurance denied or delayed something'));
    await evalOn(cdp, p, fill('#im-msg', `${UNIQUE}. QA verification submission only. No real accident. Delete after Wave 3 verification.`));
    await evalOn(cdp, p, clickAction('Continue'));
    await delay(700);
    ...
    const summary = {
      ...
      pass: {
        ...
        analyticsRowStored: Array.isArray(finalAnalyticsRows.body) && finalAnalyticsRows.body.length > 0,
        utmPersisted: rows?.[0]?.utm_source === 'google' && rows?.[0]?.utm_medium === 'cpc' && rows?.[0]?.utm_campaign === CAMPAIGN && rows?.[0]?.utm_content === 'control' && rows?.[0]?.utm_term === 'ontario_accident_benefits',
      },
    };
...
const userData = join(tmpdir(), `oar-wave3-chrome-${Date.now()}`);
